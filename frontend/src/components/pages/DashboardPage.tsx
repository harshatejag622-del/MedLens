...
export const DashboardPage: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/90 to-teal-950/40 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold tracking-wider transition-all shadow-[0_0_20px_rgba(20,184,166,0.3)] flex items-center gap-2"
          >
        ...
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              onClick={card.action}
              className={`p-5 rounded-2xl bg-[#0e1424] border ${card.border} hover:border-slate-600 transition-all cursor-pointer flex items-center justify-between group shadow-sm`}
            >
              <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="p-5 rounded-2xl bg-[#0e1424] border border-slate-800 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onNavigate('conflicts')} className="text-[11px] text-teal-400 hover:underline">
              Resolve →
            </button>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="p-2 rounded-xl bg-rose-500/10 border border-rose-500/20">
              <span className="text-[9px] text-rose-300 block font-mono">HIGH</span>
              <span className="text-base font-bold text-rose-400 font-mono">{stats?.high_conflicts ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
              <span className="text-[9px] text-amber-300 block font-mono">MEDIUM</span>
              <span className="text-base font-bold text-amber-400 font-mono">{stats?.medium_conflicts ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <span className="text-[9px] text-emerald-300 block font-mono">RESOLVED</span>
              <span className="text-base font-bold text-emerald-400 font-mono">{stats?.resolved_conflicts ?? 0}</span>
            </div>
          </div>
        ...
        <div className="p-5 rounded-2xl bg-[#0e1424] border border-slate-800 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onNavigate('review')} className="text-[11px] text-teal-400 hover:underline">
              Queue →
            </button>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
              <span className="text-[9px] text-amber-300 block font-mono">PENDING</span>
              <span className="text-base font-bold text-amber-400 font-mono">{stats?.pending_reviews ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-teal-500/10 border border-teal-500/20">
              <span className="text-[9px] text-teal-300 block font-mono">VERIFIED</span>
              <span className="text-base font-bold text-teal-400 font-mono">{stats?.verified_reviews ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-violet-500/10 border border-violet-500/20">
              <span className="text-[9px] text-violet-300 block font-mono">CORRECTED</span>
              <span className="text-base font-bold text-violet-400 font-mono">{stats?.corrected_reviews ?? 0}</span>
            </div>
          </div>
          <p className="text-[10px] text-slate-400 font-mono">
        ...
        <div className="p-5 rounded-2xl bg-[#0e1424] border border-slate-800 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-slate-400">Strict Audit</span>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="p-2 rounded-xl bg-sky-500/10 border border-sky-500/20">
              <span className="text-[9px] text-sky-300 block font-mono">AI EXTRACT</span>
              <span className="text-base font-bold text-sky-400 font-mono">{stats?.ai_extracted_items ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <span className="text-[9px] text-emerald-300 block font-mono">VERIFIED</span>
              <span className="text-base font-bold text-emerald-400 font-mono">{stats?.human_verified_items ?? 0}</span>
            </div>
            <div className="p-2 rounded-xl bg-rose-500/10 border border-rose-500/20">
              <span className="text-[9px] text-rose-300 block font-mono">REJECTED</span>
              <span className="text-base font-bold text-rose-400 font-mono">{stats?.human_rejected_items ?? 0}</span>
            </div>
          </div>
          <p className="text-[10px] text-slate-400 font-mono">
          ...
          <div className="space-y-3">
            {patients.map((pat) => (
              <div
                ...
                className="p-5 rounded-2xl bg-[#0e1424] border border-slate-800 hover:border-teal-500/50 transition-all cursor-pointer flex items-center justify-between group"
              >
                ...
                <div className="p-2 text-slate-500 group-hover:text-teal-400 group-hover:translate-x-1 transition-all">
